...
class HomePageState extends State<HomePage> {
  ...
  void click() async {
    ...
    if (this.title.length <= 0 ||
      ...
      await Firestore.instance
          .collection("Info")
          .document(select[isSelectedIndex])
          .get()
          .then((value) => {
  ...
  Future getVisitas() async {
    ...
    await Firestore.instance
        .collection("Calendar")
        .document(selectedDate.year.toString() +
            "_".toString() +
  ...
  Future getValues() async {
    ...
    for (int i = quantosPosts - 1; i >= 0; i--) {
      ...
      await Firestore.instance
        ...
        text = value.data["text"];
        title =
            value.data["title"].toString() + " - ".toString() + a.toString()[7];
        data = value.data["date"];
        hora = value.data["hour"];
        imagem = value.data["image"];
      });
  ...
  Future confirmVisit(int index) async {
    ...
    await Firestore.instance
        ...
        .document(selectedDate.year.toString() +
            "_".toString() +
            selectedDate.month.toString())
        .updateData({"X".toString() + selectedDate.day.toString(): listaConf});

    await Firestore.instance
        .collection("Logins")
        .document(visitas[index].toString())
        .get()
        .then((value) => {
    ...
    await Firestore.instance
        .collection("Logins")
        .document(visitas[index].toString())
        .updateData({"visitasConf": visitasConf});
